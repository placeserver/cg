...
export class SocketGateway implements OnGatewayInit { 
	...
	async setPointerMode(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		if(roleCheck == false) {
			...
			if(presenterCheck != null && presenterCheck?.permission_doc?.indexOf(ucode) > -1) {
				...
				await this.redis.hSet(`roomStatus:${confCode}`, "pointer", JSON.stringify({check: pointer}));
	...
	async documentMouseMark(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		const { x, y, h, w, click } = data;

		// ✅ Redis에 마우스 포인터 상태 저장
		const mouseData = { x, y, w, h };
		await this.redis.hSet(`roomStatus:${confCode}`, 'mousePosition', JSON.stringify(mouseData));
	...
	async moveViewMark(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		// const st 		= data.st;
		// const wh 		= data.wh;
		const { vh, top, st, wh } = data;

		// ✅ Redis 상태 저장
		const pointer = { vh, top, st, wh };
		await this.redis.hSet(`roomStatus:${confCode}`, 'pointer', JSON.stringify(pointer));
	...
	async getSocketIdByUcode(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		// ✅ Redis에서 socketId에 해당하는 사용자 정보 조회
		const raw = await this.redis.hGet("clientRooms", socketId);
		const parsed = raw ? JSON.parse(raw) : {};
	  
		// ✅ 응답 전송 (중복 emit 제거)
		socket.emit("getSocketIdByUcode", { ucode: parsed.ucode, socketId });
	...
	async requestPresenter(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		if(roleCheck == false) {
			...
			// 1. 대상 유저의 socketId 가져오기
			const targetSocketId = await this.redis.hGet(`userSocketMap:${confCode}`, userUcode);

			// 2. 대상 유저의 사용자 이름 가져오기
			const userInfoRaw = await this.redis.hGet(`usersMap:${confCode}`, ucode);
			const userInfo = userInfoRaw ? JSON.parse(userInfoRaw) : null;

			// 3. 소켓 인스턴스를 통해 emit 수행
			if (targetSocketId) {
				this.server.to(targetSocketId).emit("requestPresenter", {
				name: userInfo?.name,
				socketId: socket.id,
				userUcode: ucode,
				type,
	...
	async requestPresenterResult(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		if(roleCheck == false) {
			...
			const sid = await this.redis.hGet(`userSocketMap:${confCode}`, userUcode);
			if (sid) {
				this.server.to(sid).emit("requestPresenterResult", { value });
			}
		}
	};
	...
	async getStatus(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		if(roleCheck == false) {
			...
			// params['type'] = type;
			const rawParams = await this.redis.hGetAll(`roomStatus:${confCode}`);
			let params = Object.fromEntries(Object.entries(rawParams).map(([k,v]) => [k, JSON.parse(v)]));
			params['type'] = type;
	...
	async setStatus(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		const confInfo = await this.conferenceService.getConference(confCode);
		if (!confInfo || !confInfo.permission_doc.includes(ucode)) return;

		const keysToSet = ['sync', 'seminar', 'blackboard', 'screenShare', 'pdf', 'page', 'zoom'];
		for (const key of keysToSet) {
			const value = data?.mode?.[key];
			if (value !== undefined) {
			await this.redis.hSet(`roomStatus:${confCode}`, key, JSON.stringify(value));
			}
		}

		// ✅ Redis에서 현재 상태를 조회해서 모든 참여자에게 broadcast
		const rawStatus = await this.redis.hGetAll(`roomStatus:${confCode}`);
		const parsedStatus = Object.fromEntries(
		  Object.entries(rawStatus).map(([k, v]) => [k, JSON.parse(v)])
		);
	  
		this.brodcast(socket, 'setStatus', confCode, parsedStatus, true);
	...
	async getSyncData(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		// 발표자가 아닌 사용자만 동기화 상태 요청 가능
		if (confInfo && !confInfo.permission_doc.includes(ucode)) {
			const rawStatus = await this.redis.hGetAll(`roomStatus:${confCode}`);
			const parsedStatus = Object.fromEntries(
			Object.entries(rawStatus).map(([k, v]) => [k, JSON.parse(v)])
			);

			socket.emit('getSyncData', parsedStatus);
		}
	};
	...
	async joinRoom(@ConnectedSocket() socket) {
		...
		const clientsInRoom = await this.server.in(ccode).allSockets(); //socket.adapter.rooms.get(ccode);
		...
		if(numClients == 1 && confInfo?.permission_doc?.indexOf(ucode) == -1 && confInfo?.participant?.indexOf(ucode) != -1) {
			...
			setTimeout(async() =>{
				try {
					const updatedSockets = await this.server.in(ccode).allSockets();
					if (updatedSockets.size === 1) {
					  await this.conferenceService.presenterDisconnetionChange({ confCode: ccode, ucode });
					  this.brodcast(socket, 'changePresenter', ccode, { type: null }, true);
					}
				  } catch (err) {
					console.error('[Error] 발표자 자동 전환 실패:', err);
				  }
			}, 5000);
		}

		await this.redis.hSet('clientRooms', socket.id, JSON.stringify({
			ucode: ucode,
			ccode: ccode
		  }));
		...
		// ✅ roomStatus Redis 기반으로 초기화
		const exists = await this.redis.exists(`roomStatus:${ccode}`);
		if (!exists) {
			const defaultStatus = {
			  sync: true,
			  seminar: false,
			...
			for (const [key, val] of Object.entries(defaultStatus)) {
			  await this.redis.hSet(`roomStatus:${ccode}`, key, JSON.stringify(val));
			}
		  }
	...
	async leaveRoom(@ConnectedSocket() socket) {
		...
		// 1. 방에서 나가기 (socket.io-redis adapter 통해 전파됨)
		socket.leave(ccode);
		 // 2. 사용자 연결 해제 처리 (화면상 표시 등)
		await this.setUserDisconnection(socket, ccode, ucode);

		// 3. Redis 상태 정리
		await this.redis.hDel('clientRooms', socket.id);
		await this.redis.hDel(`userSocketMap:${ccode}`, ucode);
	}
	...
	async setUserConnection(@ConnectedSocket() socket, ccode: string, ucode: string) {
		...
		if(userInfo != null) {
			...
			// 2. socketId 포함된 userInfo 저장
			const userWithSocket = { ...userInfo, socket: socket.id };

			// 3. Redis 상태 저장
			await this.redis.hSet(`usersMap:${ccode}`, ucode, JSON.stringify(userWithSocket));
			await this.redis.hSet(`userSocketMap:${ccode}`, ucode, socket.id);
		}
	}
	...
	async setUserDisconnection(@ConnectedSocket() socket, ccode: string, ucode: string) {
		try{
			...
			const raw = await this.redis.hGet(`usersMap:${ccode}`, ucode);
			...
			await this.redis.hDel(`usersMap:${ccode}`, ucode);
			await this.redis.hDel(`userSocketMap:${ccode}`, ucode);
	...
	async sendAllUser(@MessageBody() data: any, @ConnectedSocket() socket, ccode: string) {
		...
		// Redis에서 사용자 목록 전부 가져오기
		const rawUsers = await this.redis.hGetAll(`usersMap:${ccode}`);
		const users = {};
		for (const [ucode, value] of Object.entries(rawUsers)) {
			try {
			  users[ucode] = JSON.parse(value);
			} catch (e) {
			  console.error(`[sendAllUser] JSON parse error for ${ucode}:`, e);
			}
		  }
	...
	async conferenceUserExit(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		if (confInfo.ucode === ucode || userInfo.role === 'admin') {
			// ✅ Redis에서 해당 유저의 socketId 조회
			const targetSocketId = await this.redis.hGet(`userSocketMap:${confCode}`, userUcode);
		
			if (targetSocketId) {
			  this.server.to(targetSocketId).emit("conferenceUserExit", {});
			} else {
			  console.warn(`[conferenceUserExit] 소켓 ID를 찾을 수 없음: userUcode=${userUcode}`);
			}
	...
	async sendMessage(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		if(roleCheck == false) {
			...
			// ✅ 클러스터 대응 브로드캐스트
			for (const receiverUcode of users) {
				const socketId = await this.redis.hGet(`userSocketMap:${confCode}`, receiverUcode);
				if (socketId) {
				this.server.to(socketId).emit("receiveMessage", sendData);
				}
			}
	...
	async changePdf(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		const confCode = this.utils.removeStrExp(data?.confCode);
		const confInfo = await this.conferenceService.getConference(confCode);
		if (confInfo.permission_doc?.includes(ucode)) {
			const pdf = data?.pdf;
		
			// ✅ Redis에 상태 저장
			await this.redis.hSet(`roomStatus:${confCode}`, 'pdf', JSON.stringify(pdf));
	...
	async sendPageChange(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		const confInfo = await this.conferenceService.getConference(confCode);
		if (confInfo.ucode === ucode || confInfo.presenter?.includes(ucode)) {
			const params = { page, pdf, zoom, t, vh, type };
		
			// ✅ Redis에 상태 업데이트
			await this.redis.hSet(`roomStatus:${confCode}`, 'page', JSON.stringify(page));
			await this.redis.hSet(`roomStatus:${confCode}`, 'pdf', JSON.stringify(pdf));
			await this.redis.hSet(`roomStatus:${confCode}`, 'zoom', JSON.stringify(zoom));
			await this.redis.hSet(`roomStatus:${confCode}`, 't', JSON.stringify(t));
			await this.redis.hSet(`roomStatus:${confCode}`, 'vh', JSON.stringify(vh));
		
			// ✅ 참여자들에게 전송 (단, type이 'notsend'가 아닌 경우)
			if (type !== 'notsend') {
	...
	async userGetWhiteboardData(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		// ✅ Redis에서 해당 사용자의 drawBuffer 불러오기
		const rawDrawBuffer = await this.redis.hGet(`whiteboardInfo:${confCode}`, ucode);
		const drawBuffer = rawDrawBuffer ? JSON.parse(rawDrawBuffer) : [];

		// ✅ 발표자의 판서 데이터를 모든 참여자에게 전송
		this.brodcast(socket, 'userGetWhiteboardData', confCode, { ucode, drawBuffer });
	}
	...
	async drawWithoutFetch(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		// ✅ Redis에 판서 데이터 저장
		await this.redis.hSet(`whiteboardInfo:${confCode}`, ucode, JSON.stringify(drawBuffer));

		// ✅ 클러스터-safe broadcast
		this.brodcast(socket, 'drawWithoutFetch', confCode, { ucode, drawBuffer });
	}
	...
	async drawToWhiteboard(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		// ✅ 클러스터-safe broadcast to participants
	...
	async getPresenterInfo(@MessageBody() data: any, @ConnectedSocket() socket) {
		...
		console.log('getPresenterInfo: ', )

		// ✅ 발표자 정보가 있는지 여부에 따라 전송 내용 변경 가능
	...
	brodcast(@ConnectedSocket() socket: Socket, event, ccode, data, self = false) {
		// socket.broadcast.to(ccode).emit(event, data);
		if (self) {
			// 송신자 포함하여 전체 전송
			this.server.to(ccode).emit(event, data); // 또는 this.server.in(ccode)
		} else {
			// 송신자를 제외한 전체 전송
			this.server.to(ccode).except(socket.id).emit(event, data);
		}
	}
	...
	usersBrodcast(confCode: string, userCodes: string[], socket, event: string, message: any) {
		...
		// 	for (let ucode of userCodes) {
		// 	const otherSocket = this.userSocketMap[confCode][ucode];
		// 	if (!otherSocket || socket.id === otherSocket.id) continue;
		// 	targets.push(otherSocket.id); // ✅ socket ID 수집
		// 	}
		// } else {
		// 	for (let ucode in this.userSocketMap[confCode]) {
		// 	const otherSocket = this.userSocketMap[confCode][ucode];
		// 	if (!otherSocket || socket.id === otherSocket.id) continue;
		// 	targets.push(otherSocket.id);
		// 	}
		// }

		// // 한 번에 다 보내기
		// if (targets.length > 0) {
		// 	this.server.to(targets).emit(event, message); // ✅ 클러스터 전파 지원
		// }
	...
	async changeDisconnetionPresenter(confCode, socket) {
		...
		// ✅ Redis에서 사용자 목록 가져오기
		const allUsersRaw = await this.redis.hGetAll(`usersMap:${confCode}`);
		const userCodes = Object.keys(allUsersRaw || {});
		if (userCodes.length === 0) return;

		let presenterChanged = false;

		for (const ucode of userCodes) {
			const check = await this.conferenceService.checkParticipant({ confCode, ucode });
			if (check === true) {
			presenterChanged = true;
			await this.conferenceService.presenterDisconnetionChange({ confCode, ucode });
			break;
			}
		}

		// ✅ 상태 초기화 → Redis로 전환
		await this.redis.hSet(`roomStatus:${confCode}`, 'seminar', 'false');
		await this.redis.hSet(`roomStatus:${confCode}`, 'blackboard', 'false');
		await this.redis.hSet(`roomStatus:${confCode}`, 'screenShare', 'false');
		await this.redis.hSet(`roomStatus:${confCode}`, 'zoom', JSON.stringify('auto'));
